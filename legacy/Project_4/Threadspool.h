...
class Net_opt {
    public:
        enum mode {SEND, RECV,RESPONSE,HTTP,HTTPS} mode;
...
void server_info(double elp, int TCP_client, int UDP_client, int current_threads, int current_pool_size) {
    cout << "Elapsed " << elp << "s ";
    cout << "ThreadPool " << current_threads + 1 << "/" << current_pool_size;
